...
Avant de reviewer, lire le fichier de conventions `.claude/rules/RULES.md` pour comprendre les standards du projet.

## Ta Base de Connaissances

Tu as acces aux standards PowerShell complets. Lire ces fichiers pour comprendre les regles :

### Nommage & Structure
- `.claude/rules/powershell/naming.md` - Verb-Noun, noms de variables interdits ($data, $temp, $i)
- `.claude/rules/powershell/parameters.md` - Attributs de validation des parametres
- `.claude/rules/powershell/modules.md` - Structure modules (Public/Private)

### Qualite du Code
- `.claude/rules/powershell/errors.md` - Gestion d'erreurs (-ErrorAction Stop dans try-catch)
- `.claude/rules/powershell/performance.md` - Patterns de performance (List<T>, .Where())
- `.claude/rules/powershell/security.md` - Securite (TryParse, Test-SafePath, pas de creds en dur)
- `.claude/rules/powershell/patterns.md` - Design patterns (Factory, Singleton, etc.)

### Anti-Patterns
- `.claude/rules/powershell/anti-patterns.md` - Patterns a eviter avec explications

### Standards UI
- `.claude/rules/powershell/ui/symbols.md` - Brackets [+][-][!][i][>][?], pas d'emoji

## Processus de Review

1. **Lire les conventions** - `.claude/rules/RULES.md` en premier
...
Pour les audits approfondis, appliquer le protocole complet defini dans `.claude/rules/common/anti-false-positives.md`.
...
Pour audits complets en 6 phases, consulter :
- `.claude/rules/common/audit-methodology.md` - Structure 6 phases
- `.claude/rules/common/anti-false-positives.md` - Protocole validation
- `.claude/rules/common/metrics-sqale.md` - Quantification dette technique
